class AES:
    def __init__(self, key: bytes):
        if len(key) != 16:
            raise ValueError("AES-128 requires a 16-byte key")
        self.sbox = [
            
        ]
        self.rcon = [
            0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
        ]
        self.expanded_key = self._key_expansion(key)

    def _key_expansion(self, key: bytes) -> list:
        w = list(key)
        for i in range(4, 44):
            temp = w[(i-1)*4 : i*4]
            if i % 4 == 0:
                temp = [self.sbox[temp[1]], self.sbox[temp[2]], self.sbox[temp[3]], self.sbox[temp[0]]]
                temp[0] ^= self.rcon[i // 4]
            w.extend([a ^ b for a, b in zip(w[(i-4)*4 : (i-3)*4], temp)])
        return w

    def _sub_bytes(self, state):
        for r in range(4):
            for c in range(4):
                state[r][c] = self.sbox[state[r][c]]

    def _shift_rows(self, state):
        state[1][0], state[1][1], state[1][2], state[1][3] = state[1][1], state[1][2], state[1][3], state[1][0]
        state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
        state[3][0], state[3][1], state[3][2], state[3][3] = state[3][3], state[3][0], state[3][1], state[3][2]

    def _xtime(self, x):
        return ((x << 1) ^ 0x1b) & 0xff if (x & 0x80) else (x << 1)

    def _mix_columns(self, state):
        for c in range(4):
            s0, s1, s2, s3 = state[0][c], state[1][c], state[2][c], state[3][c]
            state[0][c] = self._xtime(s0 ^ s1) ^ s1 ^ s2 ^ s3
            state[1][c] = self._xtime(s1 ^ s2) ^ s0 ^ s2 ^ s3
            state[2][c] = self._xtime(s2 ^ s3) ^ s0 ^ s1 ^ s3
            state[3][c] = self._xtime(s3 ^ s0) ^ s0 ^ s1 ^ s2

    def _add_round_key(self, state, round_idx):
        for r in range(4):
            for c in range(4):
                state[r][c] ^= self.expanded_key[round_idx * 16 + c * 4 + r]

    def encrypt(self, plaintext: bytes) -> bytes:
        state = [[plaintext[c * 4 + r] for c in range(4)] for r in range(4)]
        self._add_round_key(state, 0)
        
        for round_idx in range(1, 10):
            self._sub_bytes(state)
            self._shift_rows(state)
            self._mix_columns(state)
            self._add_round_key(state, round_idx)
            
        self._sub_bytes(state)
        self._shift_rows(state)
        self._add_round_key(state, 10)
        
        return bytes(state[r][c] for c in range(4) for r in range(4))